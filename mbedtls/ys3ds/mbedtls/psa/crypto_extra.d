module ys3ds.mbedtls.psa.crypto_extra;
...
import ys3ds.mbedtls.psa.crypto_compat;
...
enum MBEDTLS_PSA_KEY_SLOT_COUNT = 32;
...
alias mbedtls_psa_stats_t = mbedtls_psa_stats_s;
 ...
 */
psa_status_t mbedtls_psa_inject_entropy (const(ubyte)* seed, size_t seed_size);
 ...
 * in bytes.
 */
enum PSA_KEY_TYPE_DSA_PUBLIC_KEY = cast(psa_key_type_t) 0x4002;
 ...
 *
 */
enum PSA_KEY_TYPE_DSA_KEY_PAIR = cast(psa_key_type_t) 0x7002;

/** Whether a key type is a DSA key (pair or public-only). */
extern (D) auto PSA_KEY_TYPE_IS_DSA(T)(auto ref T type)
{
    return PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(type) == PSA_KEY_TYPE_DSA_PUBLIC_KEY;
}

enum PSA_ALG_DSA_BASE = cast(psa_algorithm_t) 0x06000400;
 ...
 *                      hash algorithm.
 */
extern (D) auto PSA_ALG_DSA(T)(auto ref T hash_alg)
{
    return PSA_ALG_DSA_BASE | (hash_alg & PSA_ALG_HASH_MASK);
}

enum PSA_ALG_DETERMINISTIC_DSA_BASE = cast(psa_algorithm_t) 0x06000500;
enum PSA_ALG_DSA_DETERMINISTIC_FLAG = PSA_ALG_ECDSA_DETERMINISTIC_FLAG;
 ...
 *                      hash algorithm.
 */
extern (D) auto PSA_ALG_DETERMINISTIC_DSA(T)(auto ref T hash_alg)
{
    return PSA_ALG_DETERMINISTIC_DSA_BASE | (hash_alg & PSA_ALG_HASH_MASK);
}

extern (D) auto PSA_ALG_IS_DSA(T)(auto ref T alg)
{
    return (alg & ~PSA_ALG_HASH_MASK & ~PSA_ALG_DSA_DETERMINISTIC_FLAG) == PSA_ALG_DSA_BASE;
}

extern (D) auto PSA_ALG_DSA_IS_DETERMINISTIC(T)(auto ref T alg)
{
    return (alg & PSA_ALG_DSA_DETERMINISTIC_FLAG) != 0;
}

extern (D) auto PSA_ALG_IS_DETERMINISTIC_DSA(T)(auto ref T alg)
{
    return PSA_ALG_IS_DSA(alg) && PSA_ALG_DSA_IS_DETERMINISTIC(alg);
}

extern (D) auto PSA_ALG_IS_RANDOMIZED_DSA(T)(auto ref T alg)
{
    return PSA_ALG_IS_DSA(alg) && !PSA_ALG_DSA_IS_DETERMINISTIC(alg);
}
...
alias PSA_ALG_IS_VENDOR_HASH_AND_SIGN = PSA_ALG_IS_DSA;
 ...
 */
enum PSA_DH_FAMILY_CUSTOM = cast(psa_dh_family_t) 0x7e;
 ...
 */
extern (D) auto PSA_KEY_DOMAIN_PARAMETERS_SIZE(T0, T1)(auto ref T0 key_type, auto ref T1 key_bits)
{
    return PSA_KEY_TYPE_IS_RSA(key_type) ? int.sizeof : PSA_KEY_TYPE_IS_DH(key_type) ? PSA_DH_KEY_DOMAIN_PARAMETERS_SIZE(key_bits) : PSA_KEY_TYPE_IS_DSA(key_type) ? PSA_DSA_KEY_DOMAIN_PARAMETERS_SIZE(key_bits) : 0;
}
 ...
 *                      correct for \p curve.
 */
mbedtls_ecp_group_id mbedtls_ecc_group_of_psa (
 ...
 */
enum MBEDTLS_PSA_KEY_ID_BUILTIN_MIN = cast(psa_key_id_t) 0x7fff0000;
 ...
 */
enum MBEDTLS_PSA_KEY_ID_BUILTIN_MAX = cast(psa_key_id_t) 0x7fffefff;

/** A slot number identifying a key in a driver.
 *
 * Values of this type are used to identify built-in keys.
 */
alias psa_drv_slot_number_t = c_ulong;
