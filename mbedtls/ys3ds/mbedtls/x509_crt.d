...
struct mbedtls_x509_crt
{
    ...
    int version_; /**< The X.509 version. (1=v1, 2=v2, 3=v3) */
    ...
    uint key_usage; /**< Optional key usage extension value: See the values in x509.h */

    mbedtls_x509_sequence ext_key_usage; /**< Optional list of extended key usage OIDs. */

    ubyte ns_cert_type; /**< Optional Netscape certificate type extension value: See the values in x509.h */
    ...
    mbedtls_x509_crt* next; /**< Next certificate in the CA-chain. */
}
...
struct mbedtls_x509_san_other_name
{
    ...
    union _Anonymous_0
    {
        ...
        _Anonymous_1 hardware_module_name;
    }
...
struct mbedtls_x509_subject_alternative_name
{
    ...
    _Anonymous_2 san; /**< A union of the supported SAN types */
}
 ...
 */
extern (D) auto MBEDTLS_X509_ID_FLAG(T)(auto ref T id)
{
    return 1 << (id - 1);
}
...
{
    uint allowed_mds; /**< MDs for signatures         */
    uint allowed_pks; /**< PK algs for public keys;
     *   this applies to all certificates
     *   in the provided chain.     */
    uint allowed_curves; /**< Elliptic curves for ECDSA  */
    uint rsa_min_bitlen; /**< Minimum size for RSA keys  */
}

enum MBEDTLS_X509_CRT_VERSION_1 = 0;
enum MBEDTLS_X509_CRT_VERSION_2 = 1;
enum MBEDTLS_X509_CRT_VERSION_3 = 2;

enum MBEDTLS_X509_RFC5280_MAX_SERIAL_LEN = 32;
enum MBEDTLS_X509_RFC5280_UTC_TIME_LEN = 15;

enum MBEDTLS_X509_MAX_FILE_PATH_LEN = 512;
 ...
 */
enum MBEDTLS_X509_MAX_VERIFY_CHAIN_SIZE = MBEDTLS_X509_MAX_INTERMEDIATE_CA + 2;
...
{
    mbedtls_x509_crt_verify_chain_item[10] items;
    uint len;
...
alias mbedtls_x509_crt_restart_ctx = void;
 ...
 */
extern __gshared const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_default;

/**
 * Expected next default profile. Recommended for new deployments.
 * Currently targets a 128-bit security level, except for allowing RSA-2048.
 */
extern __gshared const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_next;

/**
 * NSA Suite B profile.
 */
extern __gshared const mbedtls_x509_crt_profile mbedtls_x509_crt_profile_suiteb;
 ...
 * \return         A negative error code on failure.
 */
alias mbedtls_x509_crt_ext_cb_t = int function (
    void* p_ctx,
 ...
 * \return           A negative error code on failure.
 */
int mbedtls_x509_crt_parse_der_with_ext_cb (
    mbedtls_x509_crt* chain,
    const(ubyte)* buf,
    size_t buflen,
    int make_copy,
 ...
 * \return         A negative error code on failure.
 */
int mbedtls_x509_crt_parse_der_nocopy (
    mbedtls_x509_crt* chain,
    const(ubyte)* buf,
    size_t buflen);
 ...
 * \return              A negative error code on failure.
 */
alias mbedtls_x509_crt_ca_cb_t = int function (
    void* p_ctx,
 ...
 */
int mbedtls_x509_crt_check_key_usage (const(mbedtls_x509_crt)* crt, uint usage);
 ...
 */
void mbedtls_x509write_crt_set_version (mbedtls_x509write_cert* ctx, int version_);
 ...
 * \return          0 if successful, or MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int mbedtls_x509write_crt_set_ns_cert_type (
    mbedtls_x509write_cert* ctx,
    ubyte ns_cert_type);
